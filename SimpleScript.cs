...
/// </summary>
[ScriptType(name: "SimpleScript", territorys: [179, 979], guid: "d3b6a9b4-1e0e-4e0c-b7c7-ff1fce0e6cf2",
    version: "0.0.0.3", author: "Karlin", note: noteStr, updateInfo: updateInfoStr)]
public class SimpleScript : IDisposable
{
    ...
    /// </summary>
    [UserSetting("This is a test Property")]
    public int prop1 { get; set; } = 1;

    [UserSetting("Another Test Property")] public bool prop2 { get; set; } = false;

    [UserSetting("UserColorSetting")] public ScriptColor color { get; set; } = new();

    [UserSetting("EnumSetting")] public TestEnum enumSetting { get; set; }
    ...
    [ScriptMethod(EventTypeEnum.StartCasting, eventCondition: ["ActionId:124"])]
    public void DrawCircle(Event @event, ScriptAccessory accessory)
    {
        ...
        prop.Scale = new Vector2(5);
        prop.InnerScale = new Vector2(3);
        ...
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Donut, prop, dpc =>
        {
            ...
            var hue = elapsedSeconds * 0.5f % 1f;
            ...
            dpc.Scale = new Vector2(3 + hue * 3);
        });
    }

    [ScriptMethod(EventTypeEnum.StartCasting, eventCondition: ["ActionId:999999"])]
    ...
    [ScriptMethod(name: "Unconfigurable Method", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:124"], userControl: false)]
    ...
    [ScriptMethod(EventTypeEnum.StartCasting, eventCondition: ["ActionId:37010"])]
    ...
    [ScriptMethod(EventTypeEnum.ActionEffect, eventCondition: ["ActionId:124"])]
    ...
    [ScriptMethod(EventTypeEnum.ActionEffect, eventCondition: ["ActionId:99999"])]
    public void TestCreateLockOn(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            var handle = accessory.Method.VfxMethod.CreateLockOn(1, 0x40001234, new Vector4(1, 0, 0, 1), 10000,
                (ref color, ref pos, ref scale) =>
                {
                    var elapsedSeconds = (float)(DateTime.Now - start).TotalSeconds;
                    var hue = elapsedSeconds * 0.5f % 1f;
                    scale = new Vector3(hue * 1 + 0.5f);
                    color = HueToRGB(hue);
                    //pos = new(hue);
                    unsafe
                    {
                        var obj = (BattleChara*)objHandle;
                        obj->Position = new FFXIVClientStructs.FFXIV.Common.Math.Vector3(hue);
                    }
                });
        });
    ...
    public void TestCreateOmen(Event @event, ScriptAccessory accessory)
    {
        ...
        var handle = accessory.Method.VfxMethod.CreateOmen(3, new Vector3(3), accessory.Data.MyObject.Position, 0,
            new Vector4(1, 0, 0, 1), 10000,
            (ref color, ref pos, ref scale) =>
            {
                var elapsedSeconds = (float)(DateTime.Now - start).TotalSeconds;
                var hue = elapsedSeconds * 0.5f % 1f;
                color = HueToRGB(hue); //算一个rgb跑马灯颜色并赋值给颜色
                pos = new Vector4(accessory.Data.MyObject.Position, pos.W + MathF.PI / 100); //追踪游戏人物的坐标
                scale = new Vector3(2 + hue * 3); //随时间大小变化
            });
    }
    ...
    {
        accessory.Method.Mark(accessory.Data.Me, MarkType.Attack1);
        accessory.Method.Mark((uint)accessory.Data.MyObject.TargetObjectId, MarkType.Attack2, true);
    }
    ...
    {
        t = new Timer(_ => { accessory.Log.Debug("来自版本F的timer，每5秒钟提示一次"); }, null, TimeSpan.Zero,
